...
func signinHandler(w http.ResponseWriter, r *http.Request) {
	...
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		...
		"exp":      time.Now().Add(time.Hour * 24).Unix(),   //token的有效期是24小时  .Unix() 从1970年1月1号开始算，到今天为止算出个数字
	})
	...
	w.Write([]byte(tokenString))  //如果成功，写到response body里
}
